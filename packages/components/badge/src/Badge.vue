...
<template>
  <div class="u-badge">
    <div v-if="showBadge" class="u-badge-content" :class="[badgeClass, `u-badge-${type}`]" :style="_badgeStyle">{{ _value }}</div>
    <div v-else-if="isDot" class="u-badge-dot" :class="`u-badge-dot-${type}`"></div>
    <slot></slot>
  </div>
</template>
